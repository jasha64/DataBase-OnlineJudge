...
<body leftmargin="30" >
  <div class="container">
    ...
    <form method=POST action=problem_edit.php>
      ...
      <input type=hidden name=problem_id value='<?php echo $row['problem_id']?>'>
        ...
        <p align=left>
          ...
          <textarea name=source style="width:100%;" rows=1><?php echo htmlentities($row['category'],ENT_QUOTES,"UTF-8")?></textarea><br><br>
        </p>
        <div align=center>
    ...
    <?php
      ...
      $spj=intval($spj);
      $sql = "update problem set title = ?, description = ?, input = ?, output = ?, sample_input = ?, sample_output = ?, spj = ?, hint = ?, category = ?, in_date = NOW(), time_limit = ?, memory_limit = ? where problem_id = ?";

      @pdo_query($sql, $title, $description, $input, $output, $sample_input, $sample_output, $spj, $hint, $source, $time_limit, $memory_limit, $id) ;
      echo "Edit OK!";
